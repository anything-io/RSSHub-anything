...
module.exports = async (ctx) => {
    ...
    const album_name = info[3][1] ? ` - ${info[3][1]}` : '';
    const owner_name = info[3][5][2] ? `${info[3][5][2]} ` : '';

    const list = info[1];

    ctx.state.data = {
        title: `${owner_name}Google${album_name}`,
        link: url,
        item: list.filter((item) => {
            // FILTER OUT Videos:
            return Object.keys(item[15]).length <= 2;
        }).slice(0, 50).map((item) => {
            const time = new Date(item[2] * 1);
            const description = `<img src="${item[1][0]}=w${item[1][1]}-h${item[1][2]}-no" style="max-width: 100%;">`;
            return {
